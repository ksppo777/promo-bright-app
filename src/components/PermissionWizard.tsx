...
import { Layers, Clock, Bell, Settings as SettingsIcon, CheckCircle2 } from "lucide-react";
import { usePermissionChecker } from "../hooks/usePermissionChecker";
import { Capacitor } from "@capacitor/core";

export default function PermissionWizard({ onComplete }: { onComplete: () => void }) {
  const {
    isChecking,
    permissionsState,
  ...
  const stepsInfo = [
    {
      ...
      icon: Layers,
      title: "다른 앱 위에 표시",
      description: "앱이 백그라운드에 있을 때나 타이머 종료 시 화면을 즉시 깨우고 진행 상태를 오버레이로 표시하기 위해 필수적인 권한입니다.",
    ...
    },
    {
      ...
      icon: Clock,
      title: "정확한 알람 설정",
      description: "기기가 절전 모드이거나 앱이 꺼져있을 때도 사용자가 설정한 공부 알람과 목표 시간이 단 1초의 오차도 없이 정확히 울리도록 보장합니다.",
    ...
    },
    {
      ...
      icon: Bell,
      title: "알림 권한",
      description: "스마트폰 상단바에 현재 타이머 진행 상태를 실시간으로 표시하고, 중요한 학습 목표 달성 및 휴식 시간을 놓치지 않게 푸시 알림으로 알려드립니다.",
  ...
  if (isChecking) {
     return <div className="fixed inset-0 z-[9999] bg-white dark:bg-slate-900 flex items-center justify-center"></div>;
  }
    ...
    <div className="fixed inset-0 z-[9999] bg-slate-50 dark:bg-slate-900 flex flex-col justify-center items-center px-6">
      <div className="w-full max-w-md flex flex-col items-center">
        ...
        <div className="bg-white dark:bg-slate-800 w-full rounded-3xl p-8 shadow-2xl relative overflow-hidden flex flex-col items-center text-center transition-all duration-500 min-h-[380px]">
          ...
          <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-6 shadow-inner ${currentStepData.bgColor} ${!currentStepData.needsAction ? 'bg-emerald-100 dark:bg-emerald-900/30' : ''}`}>
             {!currentStepData.needsAction ? (
                <CheckCircle2 className="w-10 h-10 text-emerald-500 animate-in zoom-in duration-300" />
             ) : (
                <Icon className={`w-10 h-10 ${currentStepData.iconColor} animate-in fade-in duration-500`} />
             )}
          </div>
          
          <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-3 tracking-tight">
            {!currentStepData.needsAction ? "권한 허용됨" : currentStepData.title}
          </h2>
          
          <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed mb-8 flex-1">
            {!currentStepData.needsAction ? "기능을 위한 권한이 준비되었습니다." : currentStepData.description}
          </p>
          ...
          <div className="w-full">
            ...
            >
              {!currentStepData.needsAction ? "확인 완료" : "권한 허용하기"}
            </button>
            <p className="text-[10px] text-slate-400 font-medium mt-4">
              기기 시스템 설정 창에서 버튼을 눌러 권한을 허용해주세요.
            </p>
          </div>
        </div>
      </div>
