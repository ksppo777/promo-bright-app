...
export default function PomodoroTimer({ onSessionComplete, sessions, timerProps, books, updateSession, deleteSession, dailyGoalMinutes }: PomodoroTimerProps) {
  ...
  const toggleTimer = async () => {
    ...
    if (!isActive && mode === 'focus' && timerBookId) {
       ...
       const { getDailyLayouts, getTimetableRecords, parseMins } = await import('../lib/timetableUtils');
       ...
       if (currentBlock) {
          const hasRecord = records.find(r => r.blockId === currentBlock?.id || (r.hour !== undefined && `${String(r.hour).padStart(2,'0')}:00` === currentBlock?.id));
          if (!hasRecord) {
  ...
  const switchMode = (newMode: 'focus' | 'break') => {
    ...
    setTimeLeft(
      newMode === 'focus' 
        ? (timerMode === 'beginner' ? 25 * 60 : (expertTime.hours * 3600 + expertTime.minutes * 60 + expertTime.seconds)) 
        : (timerMode === 'beginner' ? 5 * 60 : (expertBreakTime.hours * 3600 + expertBreakTime.minutes * 60 + expertBreakTime.seconds))
    );
  };

  const handleTimerModeSelect = (newTimerMode: 'beginner' | 'expert') => {
    setIsActive(false);
    ...
    if (newTimerMode === 'beginner') {
      ...
      setTimeLeft(expertTime.hours * 3600 + expertTime.minutes * 60 + expertTime.seconds);
    }
  };

  const handleExpertTimeChange = (type: 'hours' | 'minutes' | 'seconds', value: number) => {
    setIsActive(false);
    if (mode === 'focus') {
      const updatedTime = { ...expertTime, [type]: value };
      setExpertTime(updatedTime);
      setTimeLeft(updatedTime.hours * 3600 + updatedTime.minutes * 60 + updatedTime.seconds);
    } else {
  ...
  const todayTotalSeconds = todaySessions.reduce((sum, s) => sum + (s.durationSeconds || s.durationMinutes * 60), 0) + (mode === 'focus' ? realTimeAddedSeconds : 0);
  ...
  if (isTracking) {
    if (!groupedSessions[activeTitle]) groupedSessions[activeTitle] = { totalSeconds: 0, items: [] };
    groupedSessions[activeTitle].totalSeconds += realTimeAddedSeconds;
  }

  const groupedEntries = Object.entries(groupedSessions).sort((a, b) => b[1].totalSeconds - a[1].totalSeconds);
    ...
    <div className="flex flex-col lg:flex-row gap-6 w-full max-w-5xl mx-auto transition-colors">
      ...
      <div className="lg:w-1/4 bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-xl shadow-blue-900/5 dark:shadow-none border border-blue-50 dark:border-slate-700 flex flex-col lg:min-h-[400px]">
        ...
        <div className="mb-6 flex flex-col gap-2">
          ...
          <div className="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
            <div 
              className={cn("h-full rounded-full transition-all duration-1000", dailyProgress >= 100 ? "bg-emerald-500" : "bg-indigo-500")}
      ...
      <div className="lg:w-1/2 flex flex-col items-center justify-center p-10 bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-blue-900/5 dark:shadow-none border border-blue-50 dark:border-slate-700">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm mb-6 bg-slate-50 dark:bg-slate-900/50 p-2 sm:p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50">
          ...
          {timerBookId && (books?.find(b => b.id === timerBookId)?.chapters?.length || 0) > 0 && (
            <select 
              className="flex-1 bg-white dark:bg-slate-800 border p-2 rounded-xl text-xs sm:text-sm font-bold border-slate-200 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700 dark:text-slate-300 shadow-sm"
        ...
        <div className="relative flex items-center justify-center mb-10 group w-full max-w-[18rem]">
          <svg className="w-full h-auto aspect-square transform -rotate-90" viewBox="0 0 288 288">
            <circle
              cx="144"
              cy="144"
              r="132"
              ...
              fill="transparent"
              strokeDasharray={timerMode === 'expert' ? "8 8" : "none"}
            ...
            <circle
              ...
              strokeDashoffset={2 * Math.PI * 132 * (progress / 100)}
              className={cn(
                "transition-all duration-1000 ease-linear shadow-sm",
                mode === 'focus' ? (timerMode === 'expert' ? "text-rose-500" : "text-blue-500") : "text-emerald-500"
              )}
              strokeLinecap="round"
            />
          </svg>
          <div className="absolute flex flex-col items-center justify-center px-4 w-full text-center">
            <span 
              className={cn(
                ...
                hours > 0 ? "text-[2.75rem] sm:text-[3.5rem]" : "text-6xl sm:text-7xl"
              )} 
        ...
        <div className="flex gap-4 items-center">
          <button
            ...
            className={cn(
              ...
              mode === 'focus' 
                ? timerMode === 'expert'
                  ? isActive ? "bg-slate-600 hover:bg-slate-700 shadow-slate-200 dark:shadow-none" : "bg-red-500 hover:bg-red-600 shadow-red-200 dark:shadow-none"
                  : isActive ? "bg-blue-600 hover:bg-blue-700 shadow-blue-200 dark:shadow-none" : "bg-blue-500 hover:bg-blue-600 shadow-blue-200 dark:shadow-none"
                : isActive ? "bg-blue-600 hover:bg-blue-700 shadow-blue-200 dark:shadow-none" : "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-200 dark:shadow-none"
            )}
          >
      ...
      <div className="lg:w-1/4 flex flex-col gap-4">
        <button
          ...
          className={cn(
            ...
            timerMode === 'beginner' ? "border-blue-500 ring-2 ring-blue-200 dark:ring-blue-900/30" : "border-blue-50 dark:border-slate-700 hover:border-blue-200 dark:hover:border-slate-600"
          )}
        >
        ...
        <div
          className={cn(
            ...
            timerMode === 'expert' ? "border-red-500 ring-2 ring-red-200 dark:ring-red-900/30" : "border-blue-50 dark:border-slate-700 hover:border-red-200 dark:hover:border-slate-600"
          )}
        >
          <button 
            ...
            <div className="flex gap-2 items-center bg-red-50/50 dark:bg-slate-900/50 p-3 rounded-2xl border border-red-100 dark:border-slate-700 mt-2">
              <div className="flex flex-col items-center flex-1">
                <input 
                  type="number" 
                  min="0" max="99" 
                  value={(mode === 'focus' ? expertTime : expertBreakTime).hours || ''}
                  placeholder="0"
                  onChange={(e) => handleExpertTimeChange('hours', parseInt(e.target.value, 10) || 0)}
                ...
                />
                <span className="text-[10px] uppercase font-bold text-slate-400 mt-1">H</span>
              </div>
              <span className="font-black text-slate-300 dark:text-slate-600 mb-5">:</span>
              <div className="flex flex-col items-center flex-1">
                <input 
                  type="number" 
                  min="0" max="59" 
                  value={(mode === 'focus' ? expertTime : expertBreakTime).minutes || ''}
                  placeholder="0"
                  onChange={(e) => handleExpertTimeChange('minutes', parseInt(e.target.value, 10) || 0)}
                  className="w-full bg-white dark:bg-slate-800 text-center font-bold text-red-900 dark:text-slate-100 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 text-sm focus:outline-none focus:ring-1 focus:ring-red-400 placeholder-slate-300"
                />
                <span className="text-[10px] uppercase font-bold text-slate-400 mt-1">M</span>
              </div>
              <span className="font-black text-slate-300 dark:text-slate-600 mb-5">:</span>
              <div className="flex flex-col items-center flex-1">
                <input 
                  type="number" 
                  min="0" max="59" 
                  value={(mode === 'focus' ? expertTime : expertBreakTime).seconds || ''}
                  placeholder="0"
                  onChange={(e) => handleExpertTimeChange('seconds', parseInt(e.target.value, 10) || 0)}
                ...
                />
                <span className="text-[10px] uppercase font-bold text-slate-400 mt-1">S</span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-2xl w-full max-w-sm border border-blue-50 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input 
                    ...
                    onChange={(e) => setEditSeconds(Math.max(0, parseInt(e.target.value) || 0) * 60 + (editSeconds % 60))}
                ...
                <div className="flex-1">
                  ...
                  <input 
                    ...
                    onChange={(e) => setEditSeconds(Math.floor(editSeconds / 60) * 60 + Math.max(0, Math.min(59, parseInt(e.target.value) || 0)))}
                    className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-lg"
                  />
                </div>
              </div>
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={() => {
                  if (editSeconds > 0) {
                    updateSession(sessionToEdit.id, { durationSeconds: editSeconds, durationMinutes: Math.floor(editSeconds / 60) });
                  } else {
                    deleteSession(sessionToEdit.id);
                  }
                  setEditModalOpen(false);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 dark:bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-2xl w-full max-w-sm border border-blue-50 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={async () => {
                  const { getDailyLayouts, getTimetableRecords, saveTimetableRecords, parseMins } = await import('../lib/timetableUtils');
                  const { targetDateStr, startTimeStr, bookId, chapterId } = syncConfirmData;
                  const layouts = await getDailyLayouts(targetDateStr);
                  const records = await getTimetableRecords(targetDateStr);
                  ...
                  timerProps.executeImmediateSync && timerProps.executeImmediateSync(syncConfirmData);
                  timerProps.setSyncIntention && timerProps.setSyncIntention({
                     targetDateStr,
                     startTimeStr,
                     bookId,
                     chapterId,
